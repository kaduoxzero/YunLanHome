...
public class EvaluationServiceImpl implements EvaluationService {
    ...
    public List<EvaluationResDTO> pageByTarget(EvaluationPageByTargetReqDTO evaluationPageByTargetReqDTO) {
        ...
        if (StrUtil.isBlank(jsonResult)) {
            log.error("评价系统返回结果为空，URL: {}", url);
            return Collections.emptyList();
        }

        JSONObject rootObj = JSONUtil.parseObj(jsonResult);
        JSONObject dataObj = rootObj.getJSONObject("data");
        if (dataObj == null) {
            log.error("评价系统返回数据格式异常，缺少data节点: {}", jsonResult);
            return Collections.emptyList();
        }
        ...
        return listArray.toList(EvaluationResDTO.class);
    }
