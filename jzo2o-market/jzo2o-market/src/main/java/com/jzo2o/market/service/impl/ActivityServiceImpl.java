...
public class ActivityServiceImpl extends ServiceImpl<ActivityMapper, Activity> implements IActivityService {
    ...
    public PageResult<ActivityInfoResDTO> findByPage(ActivityQueryForPageReqDTO dto) {
        ...
        List<ActivityInfoResDTO> list = page.getRecords().stream().map(e -> {
            ...
            Integer count2 = Math.toIntExact(couponWriteOffService.lambdaQuery().eq(CouponWriteOff::getActivityId, e.getId()).count());
    ...
    public ActivityInfoResDTO findById(Long id) {
        ...
        Integer count1 = Math.toIntExact(couponService.lambdaQuery().eq(Coupon::getActivityId, id).count());
        activityInfoResDTO.setReceiveNum(count1);

        //3. 根据活动id查询coupon_write_off统计当前活动的核销数量
        Integer count2 = Math.toIntExact(couponWriteOffService.lambdaQuery().eq(CouponWriteOff::getActivityId, id).count());
        activityInfoResDTO.setWriteOffNum(count2);
